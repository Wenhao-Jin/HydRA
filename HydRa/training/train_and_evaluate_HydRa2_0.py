...
from ..training.train_HydRa2_0 import get_y_pred_ProteinBERT
...
def main(args):
    ...
    train_index, val_index = list(skf2.split(Info_df_train['Sequence'], Info_df_train['RBP_flag']))[0]
    train_set=Info_df_train.iloc[train_index]
    valid_set=Info_df_train.iloc[val_index]
    test_set=Info_df_test
    ...
    print('STEP 2: Model training. Initialize models.')
    if no_secondary_structure:
    ...
    y_pred_ProteinBERT, protein_names = get_y_pred_ProteinBERT(model_ProteinBERT_generator, ProteinBERT_input_encoder, OUTPUT_SPEC, Info_df_train['Sequence'], Info_df_train['RBP_flag'], list(Info_df_train.index), start_seq_len = start_seq_len_ProteinBERT, start_batch_size = 32)
    ...
    score_df['seqSVM_seqDNN_ProteinBERT_score'] = -1
    if no_PIA==False:
        score_df['seqDNN_seqSVM_ProteinBERT_PPI_score'] = -1
        score_df['seqDNNseqSVMProteinBERT_PPI_score'] = -1 # combine intrinsic with extrinsic classifier (seqDNN+seqSVM)+NetMB      
        if no_PPA==False:
            score_df['seqDNNseqSVMProteinBERT_PIA_score'] = -1 # combine intrinsic with extrinsic classifier (seqDNN+seqSVM)+NetMB_STRING
            score_df['seqDNN_seqSVM_ProteinBERT_PIA_score'] = -1
    scores_DNN=list(score_df['seqDNN_score'])
    scores_SVM=list(score_df['seqSVM_score'])
    scores_ProteinBERT=list(score_df['ProteinBERT_score'])
    if no_PIA==False:
    ...
    if no_PIA==False:
        if no_PPA==False:
            training_set_score_df=training_set_score_df.sort_values('seqDNNseqSVMProteinBERT_PIA_score',ascending=False)
        else:
            training_set_score_df=training_set_score_df.sort_values('seqDNNseqSVMProteinBERT_PPI_score',ascending=False)
    else:
        training_set_score_df=training_set_score_df.sort_values('seqSVM_seqDNN_ProteinBERT_score',ascending=False)
...
## Prediction on the test set.
    ...
    y_pred_ProteinBERT2, protein_names2 = get_y_pred_ProteinBERT(model_ProteinBERT_generator, ProteinBERT_input_encoder, OUTPUT_SPEC, test_set['Sequence'], test_set['RBP_flag'], list(test_set.index), start_seq_len = start_seq_len_ProteinBERT, start_batch_size = 32)
    ProteinBERT_df2=pd.DataFrame(list(zip(protein_names2,y_pred_ProteinBERT2)), columns=['Protein','ProteinBERT_score'])
    ProteinBERT_df2=ProteinBERT_df2.set_index('Protein')
    ...
    Test_score_df=Test_score_df.join(ProteinBERT_df2, how='left')

    ## Calculate naive ensembling (probabilistic)
    Test_score_df['seqSVM_seqDNN_score'] = -1
    if no_PIA==False:
        Test_score_df['seqDNN_seqSVM_ProteinBERT_PPI_score'] = -1
        Test_score_df['seqDNN_seqSVM_ProteinBERT_PIA_score'] = -1
        Test_score_df['seqDNNseqSVMProteinBERT_PPI_score'] = -1 # combine intrinsic with extrinsic classifier (seqDNN+seqSVM)+NetMB
        Test_score_df['seqDNNseqSVMProteinBERT_PIA_score'] = -1 # combine intrinsic with extrinsic classifier (seqDNN+seqSVM)+NetMB_STRING
    ...
    for rix, row in Test_score_df.iterrows():
        ...
        if no_PIA==False:
            ...
            optimistic_prob0 = 1 - fdr_DNN*fdr_SVM*fdr_ProteinBERT*fdr_Net
            optimistic_prob3 = 1 - fdr_DNN*fdr_SVM*fdr_ProteinBERT*fdr_Net2
            optimistic_prob5 = 1 - fdr_in1*fdr_Net
            optimistic_prob7 = 1 - fdr_in1*fdr_Net2

        Test_score_df.loc[rix, 'seqSVM_seqDNN_ProteinBERT_score'] = optimistic_prob_intrinsic1
        if no_PIA==False:
            Test_score_df.loc[rix, 'seqDNN_seqSVM_ProteinBERT_PPI_score'] = optimistic_prob0
            Test_score_df.loc[rix, 'seqDNN_seqSVM_ProteinBERT_PIA_score'] = optimistic_prob3
            Test_score_df.loc[rix, 'seqDNNseqSVMProteinBERT_PPI_score'] = optimistic_prob5
            Test_score_df.loc[rix, 'seqDNNseqSVMProteinBERT_PIA_score'] = optimistic_prob7

    if no_PIA==False:
        if no_PPA==False:
            Test_score_df=Test_score_df.sort_values('seqDNNseqSVMProteinBERT_PIA_score',ascending=False)
        else:
            Test_score_df=Test_score_df.sort_values('seqDNNseqSVMProteinBERT_PPI_score',ascending=False)
    else:
        Test_score_df=Test_score_df.sort_values('seqSVM_seqDNN_ProteinBERT_score',ascending=False)
    ...
    colors=['tomato','mediumorchid','#f7ac5a','mediumseagreen','royalblue','blueviolet']
    pos_class_ratio=sum(Test_score_df.RBP_flag)*1.0/len(Test_score_df.RBP_flag)
    if no_PIA==False:
        if no_PPA==False:
            intrinsic_auc=draw_ROC_PRC_integrated2([(Test_score_df.seqDNN_score,Test_score_df.RBP_flag,'seqDNN'),(Test_score_df.seqSVM_score,Test_score_df.RBP_flag,'seqSVM'),(Test_score_df.ProteinBERT_score,Test_score_df.RBP_flag,'ProteinBERT'),(Test_score_df.seqSVM_seqDNN_ProteinBERT_score,Test_score_df.RBP_flag,'seqSVMseqDNNProteinBERT'),(Test_score_df.PIA_score,Test_score_df.RBP_flag,'PIA_based'),(Test_score_df.seqDNNseqSVMProteinBERT_PIA_score,Test_score_df.RBP_flag,'HydRa')], model_outdir, Comparison_name=Model_name+'_performance_on_testset', pos_class_ratio=pos_class_ratio, savefig=True, colors_list=colors)
        else:
            intrinsic_auc=draw_ROC_PRC_integrated2([(Test_score_df.seqDNN_score,Test_score_df.RBP_flag,'seqDNN'),(Test_score_df.seqSVM_score,Test_score_df.RBP_flag,'seqSVM'),(Test_score_df.ProteinBERT_score,Test_score_df.RBP_flag,'ProteinBERT'),(Test_score_df.seqSVM_seqDNN_ProteinBERT_score,Test_score_df.RBP_flag,'seqSVMseqDNNProteinBERT'),(Test_score_df.PPI_score,Test_score_df.RBP_flag,'PPI_based'),(Test_score_df.seqDNNseqSVMProteinBERT_PPI_score,Test_score_df.RBP_flag,'seqDNNseqSVMProteinBERT_PPI')], model_outdir, Comparison_name=Model_name+'_performance_on_testset', pos_class_ratio=pos_class_ratio, savefig=True, colors_list=colors)
    else:
        intrinsic_auc=draw_ROC_PRC_integrated2([(Test_score_df.seqDNN_score,Test_score_df.RBP_flag,'seqDNN'),(Test_score_df.seqSVM_score,Test_score_df.RBP_flag,'seqSVM'),(Test_score_df.ProteinBERT_score,Test_score_df.RBP_flag,'ProteinBERT'),(Test_score_df.seqSVM_seqDNN_ProteinBERT_score,Test_score_df.RBP_flag,'seqSVMseqDNNProteinBERT')], model_outdir, Comparison_name=Model_name+'_performance_on_testset', pos_class_ratio=pos_class_ratio, savefig=True, colors_list=colors)
