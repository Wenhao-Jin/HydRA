...
def run_occlusion_HydRa2(RBP_uniprotID, model_DNN, model_SVM, model_ProteinBERT_generator, ProteinBERT_input_encoder, seq_dir, k, out_dir, no_secondary_structure=False, svm_feature_table=None, scores_DNN=None, scores_SVM=None, scores_ProteinBERT=None, true_labels=None, selected_aa3mers=None, selected_aa4mers=None, selected_SS11mers=None, selected_SS15mers=None, selected_AAC=None, BioVec_name_dict=None, AAs=None, replace_value=22, start_seq_len_ProteinBERT=512):
    path=out_dir
    if not os.path.exists(path):
        os.mkdir(path)
    if not no_secondary_structure:
    ...
    if not os.path.exists(os.path.join(path, RBP_uniprotID+'_Occlusion_score_matrix_aac.xls')):
        print(RBP_uniprotID)
        occluded_DNN_scores, occluded_SVM_scores, occluded_ProteinBERT_scores, original_DNN_score, original_SVM_score, original_ProteinBERT_score, occluded_coord=get_occluding_1Dheatmap_domain_stripe_SONARp_SeqOnly_HydRa2(RBP_uniprotID, model_DNN, model_SVM, model_ProteinBERT_generator, ProteinBERT_input_encoder, k, no_secondary_structure=no_secondary_structure, svm_feature_table=svm_feature_table, scores_DNN=scores_DNN, scores_SVM=scores_SVM, scores_ProteinBERT=scores_ProteinBERT, true_labels=true_labels, selected_aa3mers=selected_aa3mers, selected_aa4mers=selected_aa4mers, selected_SS11mers=selected_SS11mers, selected_SS15mers=selected_SS15mers, selected_AAC=selected_AAC, seq_dir=seq_dir, BioVec_name_dict=BioVec_name_dict, AAs=AAs, replace_value=replace_value, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)
        delta_DNN=occluded_DNN_scores - original_DNN_score
        delta_SVM=occluded_SVM_scores - original_SVM_score
...
def get_reference_avgZscore(HydRa_df, occ_dir, col='avg_zscore_deltaSVM_deltaDNN_deltaProteinBERT'):
    ...
    for prot in prot_set:
        scores_list+=list(pd.read_table(os.path.join(occ_dir, prot+'_Occlusion_score_matrix_full_aac_addZscores.xls'))[col])
    return scores_list
...
def get_stats_dic(scores_dic):
    ...
    return stats_dic
def add_z_scores(df, stats_dic, SVM_col='seqSVM_score', DNN_col='seqDNN_score', ProteinBERT_col='ProteinBERT_score', avg_col='avg_zscore_deltaSVM_deltaDNN_deltaProteinBERT'):
...
def get_avg_zscore_stats(HydRa_score_whole_df, seqSVM_seqDNN_score_stats_dic, occ_ref_dir, occ_dir, SVM_col, DNN_col, ProteinBERT_col):
    ...
    for prot in prot_set:
        ...
        if not os.path.exists(outfile):
            occ_df=add_z_scores(occ_df, seqSVM_seqDNN_score_stats_dic, SVM_col, DNN_col, ProteinBERT_col, 'avg_zscore_deltaSVM_deltaDNN_deltaProteinBERT')
...
def plot_occlusion(RBP_uniprotID, out_dir, wind_size, annotation_file=None, annotation_file_separator=',', ens_delta_zscore_col='avg_zscore_deltaSVM_deltaDNN_deltaProteinBERT', ens_delta_zscore_track_name='Avg. Standardized Delta Scores',run_fdr_ensemble=False):
    """
    ...
    if annotation_file!=None:
        ...
        f, ax = plt.subplots(4+len(types), 1, figsize=(20, 3*(4+len(types))))
        ...
        ax[len(types)+2].set_title('Standardized Delta Scores in ProteinBERT, (Occ - origin)',fontsize='x-large')
        ...
        ax3.set_title('Standardized Delta Scores in ProteinBERT, (Occ - origin)',fontsize='x-large')
...
def main(args):
    ...
    if use_Zscore==True and ref_seq_dir!=None and out_dir2!=None:
        ...
        while cutoff>=0.05:
            ...
            prot_set_ProteinBERT=list(HydRa_score_whole_df[(HydRa_score_whole_df[ProteinBERT_col]<=cutoff) & (HydRa_score_whole_df[ProteinBERT_col]>cutoff-0.05)].index)
            prot_set_ProteinBERT=[x for x in prot_set_ProteinBERT if os.path.exists(os.path.join(out_dir2, x+'_Occlusion_score_matrix_aac.xls'))]
            shuffle(prot_set_ProteinBERT)
            prot_set_ProteinBERT=prot_set_ProteinBERT[:min(100,len(prot_set_ProteinBERT))]
            
            scores_dic[SVM_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]=[]
            scores_dic[DNN_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]=[]
            scores_dic[ProteinBERT_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]=[]
            scores_dic[seq_score_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]=[]

            if run_fdr_ensemble:
                prot_set_FdrEns=list(HydRa_score_whole_df[(HydRa_score_whole_df[seq_score_col]<=cutoff) & (HydRa_score_whole_df[seq_score_col]>cutoff-0.05)].index)
                prot_set_FdrEns=[x for x in prot_set_FdrEns if os.path.exists(os.path.join(out_dir2, x+'_Occlusion_score_matrix_aac.xls'))]
                shuffle(prot_set_FdrEns)
                prot_set_FdrEns=prot_set_FdrEns[:min(100,len(prot_set_FdrEns))]
                scores_dic[seq_score_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]=[]

            if no_secondary_structure:
                if len(prot_set_DNN)>0:
                    wrap_run_occlusion_HydRa2(prot_set_DNN, ref_seq_dir, out_dir2, selected_aa3mers_file, selected_aa4mers_file, selected_AAC_file, combined_selected_feature, autoEncoder_CNN_file, autoEncoder_Dense_file, BioVec_weights_add_null, maxlen, max_seq_len, seqDNN_modelfile_stru, seqDNN_modelfile_weight, seqSVM_modelfile, proteinBERT_modelfile, model_dir, model_name, no_secondary_structure, k, BioVec_name_dict, AAs, scores_DNN, scores_SVM, true_labels, n_annotations=n_annotations, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)            
                if len(prot_set_SVM)>0:
                    wrap_run_occlusion_HydRa2(prot_set_SVM, ref_seq_dir, out_dir2, selected_aa3mers_file, selected_aa4mers_file, selected_AAC_file, combined_selected_feature, autoEncoder_CNN_file, autoEncoder_Dense_file, BioVec_weights_add_null, maxlen, max_seq_len, seqDNN_modelfile_stru, seqDNN_modelfile_weight, seqSVM_modelfile, proteinBERT_modelfile, model_dir, model_name, no_secondary_structure, k, BioVec_name_dict, AAs, scores_DNN, scores_SVM, true_labels, n_annotations=n_annotations, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)            
                if len(prot_set_ProteinBERT)>0:
                    wrap_run_occlusion_HydRa2(prot_set_ProteinBERT, ref_seq_dir, out_dir2, selected_aa3mers_file, selected_aa4mers_file, selected_AAC_file, combined_selected_feature, autoEncoder_CNN_file, autoEncoder_Dense_file, BioVec_weights_add_null, maxlen, max_seq_len, seqDNN_modelfile_stru, seqDNN_modelfile_weight, seqSVM_modelfile, proteinBERT_modelfile, model_dir, model_name, no_secondary_structure, k, BioVec_name_dict, AAs, scores_DNN, scores_SVM, true_labels, n_annotations=n_annotations, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)            
                if run_fdr_ensemble:
                    if len(prot_set_FdrEns)>0:
                        wrap_run_occlusion_HydRa2(prot_set_FdrEns, ref_seq_dir, out_dir2, selected_aa3mers_file, selected_aa4mers_file, selected_AAC_file, combined_selected_feature, autoEncoder_CNN_file, autoEncoder_Dense_file, BioVec_weights_add_null, maxlen, max_seq_len, seqDNN_modelfile_stru, seqDNN_modelfile_weight, seqSVM_modelfile, proteinBERT_modelfile, model_dir, model_name, no_secondary_structure, k, BioVec_name_dict, AAs, scores_DNN, scores_SVM, true_labels, n_annotations=n_annotations, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)            
            else:
                if len(prot_set_DNN)>0:
            ...
            for prot in prot_set_ProteinBERT:
                scores_dic[ProteinBERT_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]+=list(pd.read_table(os.path.join(out_dir2, prot+'_Occlusion_score_matrix_aac.xls'))['delta_ProteinBERT'])

            if run_fdr_ensemble:
                wrap_run_ensemble(prot_set_FdrEns, out_dir2, scores_DNN=scores_DNN, scores_SVM=scores_SVM, scores_ProteinBERT=scores_ProteinBERT, true_labels=true_labels)
                for prot in prot_set_FdrEns:
                    scores_dic[seq_score_col][str(int(round(cutoff-0.05,2)*100))+'-'+str(int(cutoff*100))]+=list(pd.read_table(os.path.join(out_dir2, prot+'_Occlusion_score_matrix_full_aac_fdrEnsemble.xls'))['delta_fdr_ens'])
        ...
        with open(os.path.join(out_dir,model_name+'_Occlusion_maps_seqSVM_seqDNN_ProteinBERT_scores_statistics.pkl'), 'wb') as f:
            pickle.dump(seqSVM_seqDNN_score_stats_dic, f, pickle.HIGHEST_PROTOCOL)

        ## generate
        avg_zscore_stats_dic=get_avg_zscore_stats(HydRa_score_whole_df, seqSVM_seqDNN_score_stats_dic, out_dir2, out_dir2, SVM_col, DNN_col, ProteinBERT_col)
        with open(os.path.join(out_dir,model_name+'_Occlusion_maps_seqSVM_seqDNN_ProteinBERT_avg_zscores_statistics.pkl'), 'wb') as f:
    ...
    if no_secondary_structure:
        wrap_run_occlusion_HydRa2(RBPs, seq_dir, out_dir, selected_aa3mers_file, selected_aa4mers_file, selected_AAC_file, combined_selected_feature, autoEncoder_CNN_file, autoEncoder_Dense_file, BioVec_weights_add_null, maxlen, max_seq_len, seqDNN_modelfile_stru, seqDNN_modelfile_weight, seqSVM_modelfile, proteinBERT_modelfile, model_dir, model_name, no_secondary_structure, k, BioVec_name_dict, AAs, scores_DNN, scores_SVM, true_labels, n_annotations=n_annotations, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)                
    else:
        wrap_run_occlusion_HydRa2(RBPs, seq_dir, out_dir, selected_aa3mers_file, selected_aa4mers_file, selected_AAC_file, combined_selected_feature, autoEncoder_CNN_file, autoEncoder_Dense_file, BioVec_weights_add_null, maxlen, max_seq_len, seqDNN_modelfile_stru, seqDNN_modelfile_weight, seqSVM_modelfile, proteinBERT_modelfile, model_dir, model_name, no_secondary_structure, k, BioVec_name_dict, AAs, scores_DNN, scores_SVM, true_labels, selected_SS11mers_file=selected_SS11mers_file, selected_SS15mers_file=selected_SS15mers_file, n_annotations=n_annotations, start_seq_len_ProteinBERT=start_seq_len_ProteinBERT)            
    ...
    if use_Zscore==True:
        avg_zscore_stats_dic=get_avg_zscore_stats(HydRa_score_whole_df, seqSVM_seqDNN_score_stats_dic, out_dir2, out_dir, SVM_col, DNN_col, ProteinBERT_col)
        ...
        for RBP in RBPs:
            if run_fdr_ensemble:
                plot_occlusion(RBP, out_dir, k, ens_delta_zscore_col='zscore_deltaFdrEns', ens_delta_zscore_track_name='Zscore of Delta FDR ensemble score', run_fdr_ensemble=True)
            else:
